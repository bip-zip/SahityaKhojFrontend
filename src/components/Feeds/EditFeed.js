...
//   get post
const getPosts=()=>{
    axios.get("http://localhost:8080/api/feeds/feed/" + feedId).then(result => {
  ...
  const submitForm = (e) => {
      ...
      axios.put("http://localhost:8080/api/feeds/update", data, config).then(result => {
          console.log(result)
          if (result.data.status) {
            ...
            <div className="container bg-white shadow-sm rounded my-1 p-5">
              ...
              <div className="form-floating mb-3">
                ...
                <div className="my-3 d-flex justify-content-between">
                  <div className="">
                    <button className="btn btn-outline-danger " onClick={()=>{navigate('/feeds')}} >Discard</button>
                  </div>
                  <div className=" btn-group">
                    <button className="btn btn-purple text-white" onClick={submitForm}>Update</button>
                  </div>
                </div>
              </form>
            </div>
